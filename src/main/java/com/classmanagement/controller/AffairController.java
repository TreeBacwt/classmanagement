...
public class AffairController {
    ...
    public Result addAffair(Affair affair) {
        Integer addAffair = affairService.addAffair(affair);
        if (addAffair != 0) {
            return Result.success("班级事务添加成功！", affair);
        } else return Result.fail("班级事务添加失败，请稍后再试！");
    ...
    @GetMapping("/count")
    public Result getAffairsCount() {
        Integer count = affairService.queryAffairsCount();
        if (count != 0) {
            return Result.success("总数查询成功！", count);
